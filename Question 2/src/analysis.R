...
Student = read.csv("C:\\Users\\sailo\\OneDrive\\Documents\\Assignment\\2\\data_clean\\clean_StudentsData.csv")
Student
...
plot(Student_Data$gender, Student_Data$reading.score,
     xlab = "SEX", ylab = "READING SCORE",main="BOXPLOTS OF READING SCORE VS GENDER",col.main="blue", col=(c("orange","green")))

#2.3
Student_Data$gender <- as.factor(Student_Data$gender)
plot(Student_Data$gender, Student_Data$writing.score,
     xlab = "SEX", ylab = "WRITING SCORE",main="BOXPLOTS OF WRITING SCORE VS GENDER",col.main="red", col=(c("green","yellow")))
...
ggplot(data = Student_Data, aes(x = math.score, y = reading.score, col = gender)) +
  ...
  scale_color_manual(values = c("male" = "black", "female" = "gold")) +
...
hist(Student_Data$reading.score,xlim=c(0,100), col=5, xlab = "READING SCORE", main = "Distribution Reading Score")
hist(Student_Data$writing.score,xlim=c(0,100), col=1, breaks=10, xlab = "WRITING", main = "Distribution writing Score")
